...
public class LoggingAspect {
	...
	@AfterThrowing(pointcut = "execution(* com.tripplanner.repository.*Impl.*(..))", throwing = "exception")	
	public void logExceptionFromDAO(Exception exception) throws Exception {
		log(exception);
	}
	
	@AfterThrowing(pointcut = "execution(* com.tripplanner.service.*Impl.*(..))", throwing = "exception")	
